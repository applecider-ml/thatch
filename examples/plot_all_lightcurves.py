...
FILTER_COLORS = {
    "F225W": "#8b00ff", "F275W": "#6a0dad", "F336W": "#4400cc",
    "F438W": "#0044cc", "F475W": "#0077ff", "F555W": "#00aa44",
    "F606W": "#55aa00", "F625W": "#88aa00", "F775W": "#cc6600",
    "F814W": "#cc3300", "F850LP": "#aa0000",
    "F098M": "#990044", "F105W": "#993300", "F110W": "#aa4400",
    "F125W": "#bb5500", "F140W": "#cc6600", "F153M": "#cc7700",
    "F160W": "#dd0000",
    "F435W": "#0055dd", "F658N": "#bb4400", "F502N": "#00bb66",
}
...
def load_and_filter(csv_path, mjd_ref):
    ...
    mask = df["ab_mag"].notna() & (df["ab_mag"] < 30) & (df["ab_mag_err"].notna()) & (df["ab_mag_err"] < 2)
    df = df[mask].copy()
...
def plot_single(obj_name, meta, ax=None, standalone=True):
    ...
    for i, filt in enumerate(filters_sorted):
        ...
        ax.errorbar(
            sub["phase"], sub["ab_mag"], yerr=sub["ab_mag_err"],
            fmt="o", ms=4, capsize=2, label=filt,
            color=_color_for(filt, i), alpha=0.85, lw=0.8,
        )
    ...
    ax.legend(fontsize=6, ncol=max(1, len(filters_sorted) // 4), loc="best",
    ...
    if standalone:
        fig.tight_layout()
        for ext in ("pdf", "png"):
            fig.savefig(os.path.join(FIG_DIR, f"{obj_name}_lightcurve_all.{ext}"),
...
def main():
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(FIG_DIR, f"THATCH_lightcurves_summary.{ext}"),
                    dpi=200, bbox_inches="tight")
    plt.close(fig)
