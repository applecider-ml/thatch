...
def main():
    ...
    if len(matched) > 0:
        print(
            matched[["filter", "mjd_pub", "mag_pub", "mag_ours", "source"]].to_string(index=False)
        )
    ...
    for filt, grp in matched.groupby("filter"):
        ...
        ax1.errorbar(
            grp["mag_pub"],
            grp["mag_ours"],
            xerr=grp["mag_pub_err"],
            yerr=grp["mag_ours_err"],
            fmt="o",
            color=color,
            markersize=7,
            label=filt,
